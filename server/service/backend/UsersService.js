...
export const update = async (req) => {
  ...
  try {
    const response = (await Promise.all(users.map(async e =>  {
      ...
      // update file profile
      if(existsSync("./public/profile/"+users?.UsersDetails?.profile) && users?.UsersDetails?.profile != "default.jpg" && req.files && req.files.length > 0) unlink("./public/profile/"+users?.UsersDetails?.profile, err => {
