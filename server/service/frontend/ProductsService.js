...
export const findAll = async (req) => {
  ...
  const products = await Products.findAll({...where, include:[{ model: ImageProducts, },{model: Diskon},{ model: Categories, where: (categories) ? { 
    nama: categories
  } : {}}], paranoid ,limit, offset, order: [["id","DESC"],[ImageProducts,"id","ASC"]]})   
